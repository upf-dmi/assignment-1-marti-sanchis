...
set.seed(123)
library(pacman)
p_load(readr, dplyr, DataExplorer, zCompositions, here, magrittr, ggplot2, tidyr, GGally, mice)
...
- Secondly we plot barplots for the categorical variables. We can see that many variables' levels are unbalanced. For example, fasting_bs, rest_ecg, slope, chest_pain, thalassemia and vessel count
...
numerical_long %>% ggplot(aes(sample = log(Value))) +
  geom_qq() +
...
ggpairs(heart_disease_imputed,
        columns = c("Age", "RestingBloodPressure", "Cholesterol", "MaxHeartRate", "STDepression"),
        aes(color = HeartDiseaseDiagnosis),
        progress = FALSE) +  # Aquí pasas el mapeo de color correctamente
  theme_minimal() +
